...
public class InOutEvents extends ModelEventDelegate<MInOut>{
	
	public InOutEvents(MInOut po, Event event) {
		super(po, event);
	}
		...
		if (io.getReversal_ID() == 0)
		{
			if(docType.get_ValueAsBoolean("IsControlNoDocument")) {
				...
				if (io.get_Value("LVE_ControlNumber") == null) {
					...
					MSequence seq = new MSequence(Env.getCtx(), controlNoSequence_ID, io.get_TrxName());
					controlSequence = MSequence.getDocumentNoFromSeq(seq, io.get_TrxName(), io);

					Query query = new Query(Env.getCtx(), MInOut.Table_Name, where + "AND LVE_ControlNumber=?", io.get_TrxName());
					...
					if(seq.get_ValueAsBoolean("IsLimitControlNo")) {
						int currentNext = getSequence(seq, io.getAD_Org_ID())-1;
						int qtytonotified = getQtyBeforeNotified(seq, io.getAD_Org_ID());
						int limitseqno = getLimitSequence(seq, io.getAD_Org_ID());
					...
					boolean existCN = new Query(Env.getCtx(), MInOut.Table_Name, where + "AND LVE_ControlNumber=?", io.get_TrxName()).setParameters(io.getAD_Org_ID(), io.getC_BPartner_ID(), io.get_ID(), io.get_Value("LVE_ControlNumber")).count() > 0;
					if (existCN) {
