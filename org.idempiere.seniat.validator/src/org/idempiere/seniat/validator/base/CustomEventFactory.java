...
package org.idempiere.seniat.validator.base;
...
public abstract class CustomEventFactory extends AbstractEventHandler implements IEventTopics {
	...
	private void execHandler(Event event, String eventType, EventHandlerWrapper eventHandlerWrapper) {
		if (eventType.equals(eventHandlerWrapper.getEventTopic())) {
			if (eventHandlerWrapper.getFilter() != null) {
				...
				}else if(isFactsHandler(eventType)) {
	...
	private void execEventHandler(Event event, EventHandlerWrapper eventHandlerWrapper, Object data) {
		...
		LoginEventData eventData = null;
		FactsEventData factseventData = null;
		if(event.getTopic().equalsIgnoreCase(IEventTopics.AFTER_LOGIN))
		{
			eventData = getEventData(event);
		}
		if(event.getTopic().equalsIgnoreCase(IEventTopics.ACCT_FACTS_VALIDATE))
		{
			factseventData = getEventData(event);
		}

		if (data instanceof PO)
		{
			if(eventData != null)
				customEventHandler.doHandleEvent((PO) data, event, eventData);
			else if(factseventData != null)
				customEventHandler.doHandleEvent((PO) data, event, factseventData);
			else
	...
	private boolean isFactsHandler(String eventTopic) {
		return (IEventTopics.ACCT_FACTS_VALIDATE.equals(eventTopic));
	}
