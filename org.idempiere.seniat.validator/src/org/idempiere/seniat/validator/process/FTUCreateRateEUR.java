...
@Process
public class FTUCreateRateEUR extends CustomProcess {
 ...
 // Método para verificar si ya existe una tasa creada hoy
    ...
    protected String doIt() throws Exception {
        ...
        try {
            ...
            if (dateElement != null) {
                ...
                try {
                    ...
                    throw new AdempiereUserError("Error al convertir la fecha: " + fecha, e);
                }
            } else {
                throw new AdempiereUserError("No se encontró la fecha en la página.");
            }

            // Extraer la tasa de cambio del euro
            Elements exchangeRateElements = doc.select("#euro .col-sm-6.centrado strong");
            if (!exchangeRateElements.isEmpty()) {
                ...
                log.warning("Tipo de cambio EUR: " + exchangeRateText);
                ...
                int precision = MSysConfig.getIntValue("FTU_EXCHANGE_RATE_PRECISION_EUR", 2, clientId, Env.getAD_Org_ID(getCtx()));
                log.warning("Precisión obtenida de SysConfig: " + precision);
                String roundingModeConfig = MSysConfig.getValue("FTU_EXCHANGE_RATE_ROUNDING_MODE_EUR", "HALF_UP", clientId, Env.getAD_Org_ID(getCtx()));
                ...
                try {
                    ...
                    log.warning("Método de redondeo no válido en SysConfig. Usando HALF_UP por defecto.");
            ...
            throw new AdempiereUserError("Error al obtener los datos de la página: " + url, e);
        }
        ...
        return msg; // Retornar el mensaje de éxito
    }
}